...
b32 skeletonIsCompatible(CachedPageEntry *page)
{
    AssertNonNull(page);
...
ArchetypeDiff SkeletonCreature = {
    U"Scheletro"_W, skeletonIsCompatible, 
    skeletonCreatureGS, skeletonCreatureAS, skeletonCreatureAC, archetypeSensesStub,
...
b32 zombieIsCompatible(CachedPageEntry *page)
{
    AssertNonNull(page);
...
ArchetypeDiff ZombieCreature = {
    U"Zombi"_W, zombieIsCompatible, 
    zombieCreatureGS, zombieCreatureAS, zombieCreatureAC, archetypeSensesStub,
...
ArchetypeDiff BloodySkeletonCreature = {
    U"Scheletro Insanguinato"_W, skeletonIsCompatible, 
    bloodySkeletonCreatureGS, bloodySkeletonCreatureAS, bloodySkeletonCreatureAC,
...
ArchetypeDiff BurningSkeletonCreature = {
    U"Scheletro Bruciante"_W, skeletonIsCompatible, 
    burningSkeletonCreatureGS, burningSkeletonCreatureAS, burningSkeletonCreatureAC,
...
ArchetypeDiff FastZombieCreature = {
    U"Zombi Rapido"_W, zombieIsCompatible, 
    fastZombieCreatureGS, fastZombieCreatureAS, fastZombieCreatureAC, archetypeSensesStub,
...
b32 vampireIsCompatible(CachedPageEntry *page)
{
    AssertNonNull(page);
    
    if(page->hitDice < 5)                            { return FALSE; }
    if(ls_utf32AreEqual(page->type, U"Non Morto"_W)) { return FALSE; }
...
{ 
    ...
    if(ls_utf32AreEqual(page->size, U"Minuta"_W))       { ls_utf32Append(melee, U"1"_W); }
    if(ls_utf32AreEqual(page->size, U"Minuscola"_W))    { ls_utf32Append(melee, U"1d2"_W); }
    if(ls_utf32AreEqual(page->size, U"Piccola"_W))      { ls_utf32Append(melee, U"1d3"_W); }
    if(ls_utf32AreEqual(page->size, U"Media"_W))        { ls_utf32Append(melee, U"1d4"_W); }
    if(ls_utf32AreEqual(page->size, U"Grande"_W))       { ls_utf32Append(melee, U"1d6"_W); }
    if(ls_utf32AreEqual(page->size, U"Enorme"_W))       { ls_utf32Append(melee, U"1d8"_W); }
    if(ls_utf32AreEqual(page->size, U"Mastodontica"_W)) { ls_utf32Append(melee, U"2d6"_W); }
    if(ls_utf32AreEqual(page->size, U"Colossale"_W))    { ls_utf32Append(melee, U"2d8"_W); }
...
ArchetypeDiff VampireCreature = {
    U"Vampiro"_W, vampireIsCompatible, 
    vampireCreatureGS, vampireCreatureAS, vampireCreatureAC, vampireCreatureSenses,
...
b32 exoskeletonIsCompatible(CachedPageEntry *page)
{
    AssertNonNull(page);
    
    if(ls_utf32AreEqual(page->type, U"Parassita"_W) && 
       !ls_utf32Contains(page->subtype, U"Incorporeo"_W)) { return TRUE; }
...
ArchetypeDiff ExoskeletonCreature = {
    U"Esoscheletro"_W, exoskeletonIsCompatible,
    exoskeletonGS, exoskeletonAS, exoskeletonAC, archetypeSensesStub,
